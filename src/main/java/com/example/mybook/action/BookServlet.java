...
public class BookServlet extends HttpServlet {
        ...
        switch (type){
            ...
            case "remove":
                ...
                try {
                    ...
                    if(count > 0){
                        out.println("<script>alert('删除书籍成功');location.href='book.let?type=query&pageIndex=1';</script>");
                    }else{
                        out.println("<script>alert('删除书籍失败');location.href='book.let?type=query&pageIndex=1';</script>");
                    }
                } catch (Exception e) {
                    e.printStackTrace();
    ...
    private void add(HttpServletRequest req, HttpServletResponse resp,  PrintWriter  out) throws Exception {
        ...
        if(count > 0)
        {
            ...
            out.println("<script>alert('添加书籍成功');location.href='book_add.jsp';</script>");
        }
    }
}
